<template>
  <div class="card mb-4">
    ...
    <h3 class="card-footer small text-muted">
      {{ totalText }} : {{ totalAmount }}원
    </h3>
  ...
  setup() {
    ...
    const getInfo = async () => {
      try {
        ...
        if (!res.data) {
          return console.log("일치하는 id 검색 실패");
        }
        ...
        // 가져온 데이터가 있는지 확인
        isUserinfoGot.value =
          currentUserInfo.value.income.length > 0 &&
          currentUserInfo.value.expense.length > 0;
    ...
    const arrangeInfo = async () => {
      ...
      currentUserInfo.value.income.forEach((curIncome) => {
        if (incomeMap.has(curIncome.category)) {
          incomeMap.set(
            curIncome.category,
            incomeMap.get(curIncome.category) + curIncome.amount
          );
        }
      });
      ...
      currentUserInfo.value.expense.forEach((curExpense) => {
        if (expenseMap.has(curExpense.category)) {
          expenseMap.set(
            curExpense.category,
            expenseMap.get(curExpense.category) + curExpense.amount
          );
        }
      });
    ...
    const updateChart = () => {
      // expense, income내역 추출
      const expenseData = currentUserInfo.value.expense.map(
        (item) => item.amount
      );
      const incomeData = currentUserInfo.value.income.map(
        (item) => item.amount
      );
      ...
      myDoughnutChart = new Chart(ctx, {
        ...
        data: {
          labels:
            selectedChart.value === "income"
              ? labelCategory.incomeCategory
              : labelCategory.expenseCategory,
          datasets: [
            {
              label: "#금액",
              data:
                selectedChart.value === "income"
                  ? curUserChartVal.curUserIncome
                  : curUserChartVal.curUserExpense,
              backgroundColor: [
                "rgba(255, 99, 132, 0.2)",
                "rgba(54, 162, 235, 0.2)",
                "rgba(255, 206, 86, 0.2)",
